...
const RULES: Record<string, (r: Rec, ctx?: EndCtx) => string[]> = {
  "04_CS_Import": (r, ctx) => {
    ...
    const reExp = isOne(r.re_export, "Yes") || /^Re-Export/i.test(String(r.job_type ?? ""));
    if (reExp && ctx && !ctx.hasExport.has(jn)) m.push("Re-Export: ยังไม่มีรายการนี้ที่ tab Export");
    return m;
  },
  "05_CS_Export": (r, ctx) => {
    const m: string[] = [];
